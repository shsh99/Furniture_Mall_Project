...
import MemberCouponService from "../../../service/admin/coupon/MemberCouponService";
...
function MemberCouponList() {
  // todo: 변수 정의
  // 공통코드 배열 변수 정의
  const [mCoupon, setMCoupon] = useState<Array<IMemberCoupon>>([]);
  ...
  // todo: 함수 정의
  useEffect(() => {
    retrieveMemberCoupon(); // 전체 조회
  }, [page, pageSize]);

  //   전체조회 함수
  const retrieveMemberCoupon = () => {
    MemberCouponService.getAll(page - 1, pageSize) // 벡엔드 전체조회요청
      .then((response: any) => {
        const { mCoupon, totalPages } = response.data;
        // 저장
        setMCoupon(mCoupon);
        setCount(totalPages);
        // 로그 출력
